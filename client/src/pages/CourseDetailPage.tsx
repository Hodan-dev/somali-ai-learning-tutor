    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-blue-100 bg-white p-5">
        ...
        <div className="mt-4 max-w-md">
          <CourseProgressRow title="Horumar" progress={course.progress} />
        </div>
        {course.currentLessonId && (
          <Link
            to={`/app/lessons/${course.currentLessonId}`}
            className="mt-4 inline-flex rounded-lg bg-sea px-4 py-2 text-sm font-semibold text-white hover:bg-sea-dark"
          >
            Continue
          </Link>
        )}
      ...
      <div className="space-y-4">
        {course.modules.map((mod) => (
          <section key={mod.id} className="rounded-2xl border border-blue-100 bg-white p-5">
            <h2 className="truncate font-display text-base font-semibold text-ink">{mod.title}</h2>
